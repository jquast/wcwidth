...
* `wcwidth_clip_u8()`_ rejects the unsupported sequences described above; every other sequence but
  ...
  matches Python's `clip()`_, SGR included, and Python's `clip()`_ depends on that: it calls here
  first and runs its own implementation only on ``WCWIDTH_ERROR_UNSUPPORTED``.
