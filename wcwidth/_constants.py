...
from .table_term_programs import KNOWN_TERMINALS, TERM_PROGRAM_ALIASES
...
# Map lowercase TERM_PROGRAM / alias values to canonical terminal names.
# Generated by bin/update-tables.py from ucs-detect YAML data files.
_TERM_PROGRAM_ALIASES = TERM_PROGRAM_ALIASES

# Canonical terminal names known to have override data.
# A resolved terminal that is not in this set is treated as unrecognized.
# Generated by bin/update-tables.py from ucs-detect YAML data files.
_KNOWN_TERMINALS: frozenset[str] = KNOWN_TERMINALS
...
def _resolve_terminal(term_program=None):
    ...
    :param term_program: Terminal identifier string such as a TERM_PROGRAM value.
        If None, read the ``TERM_PROGRAM`` environment variable.
    ...
    canonical = _TERM_PROGRAM_ALIASES.get(key, key)
    if canonical not in _KNOWN_TERMINALS:
        return None
    return canonical
