...
static PyObject *
wrap_impl(PyObject *self, PyObject *args, PyObject *kwargs)
{
    PyObject *text_obj;
    int wrap_width = 70;
    PyObject *control_codes_obj = NULL;
    int tabsize = 8;
    int expand_tabs = 1;
    int replace_whitespace = 1;
    int ambiguous_width = 1;
    PyObject *term_program_obj = NULL;
    PyObject *initial_indent_obj = NULL;
    PyObject *subsequent_indent_obj = NULL;
    int fix_sentence_endings = 0;
    int break_long_words = 1;
    int break_on_hyphens = 1;
    int drop_whitespace = 1;
    PyObject *max_lines_obj = NULL;
    PyObject *placeholder_obj = NULL;
    int propagate_sgr = 1;
    static char *kwlist[] = {"text", "width", "control_codes", "tabsize",
                             "expand_tabs", "replace_whitespace",
                             "ambiguous_width", "term_program",
                             "initial_indent", "subsequent_indent",
                             "fix_sentence_endings", "break_long_words",
                             "break_on_hyphens", "drop_whitespace",
                             "max_lines", "placeholder", "propagate_sgr", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "U|iOippiOOOppppOOp", kwlist,
                                     &text_obj, &wrap_width, &control_codes_obj, &tabsize,
                                     &expand_tabs, &replace_whitespace, &ambiguous_width,
                                     &term_program_obj, &initial_indent_obj,
                                     &subsequent_indent_obj, &fix_sentence_endings,
                                     &break_long_words, &break_on_hyphens, &drop_whitespace,
                                     &max_lines_obj, &placeholder_obj, &propagate_sgr)) {
        return NULL;
    }
    if (PyTuple_GET_SIZE(args) > 2) {
        PyErr_Format(PyExc_TypeError,
                     "wrap() takes 2 positional arguments but %zd were given",
                     PyTuple_GET_SIZE(args));
        return NULL;
    }

    if (wrap_width <= 0) {
        PyErr_Format(PyExc_ValueError, "invalid width %d (must be > 0)", wrap_width);
        return NULL;
    }
    ...
    /* The C wrap cannot propagate strict-mode errors, which the pure
     * implementation raises as ValueError. */
    if (mode == WCWIDTH_STRICT) {
        return call_pure_function("wcwidth.textwrap", "wrap", args, kwargs);
    }

    int max_lines = 0;
    if (max_lines_obj != NULL && max_lines_obj != Py_None) {
        max_lines = (int)PyLong_AsLong(max_lines_obj);
        if (max_lines == -1 && PyErr_Occurred()) {
            return NULL;
        }
    }

    /* textwrap raises ValueError when the placeholder cannot fit.  The C
     * library signals this by failing the wrap, so check it here first. */
    if (max_lines_obj != NULL && max_lines_obj != Py_None) {
        PyObject *indent_obj = (max_lines > 1) ? subsequent_indent_obj : initial_indent_obj;
        Py_ssize_t indent_len = 0;
        if (indent_obj != NULL) {
            if (!PyUnicode_Check(indent_obj)) {
                PyErr_SetString(PyExc_TypeError, "indent must be a string");
                return NULL;
            }
            indent_len = PyUnicode_GET_LENGTH(indent_obj);
        }
        Py_ssize_t placeholder_len;
        if (placeholder_obj == NULL) {
            placeholder_len = 5; /* ' [...]' lstrip -> '[...]' */
        } else {
            if (!PyUnicode_Check(placeholder_obj)) {
                PyErr_SetString(PyExc_TypeError, "placeholder must be a string");
                return NULL;
            }
            placeholder_len = PyUnicode_GET_LENGTH(placeholder_obj);
            Py_ssize_t skip = 0;
            for (; skip < placeholder_len; skip++) {
                uint32_t ucs = PyUnicode_ReadChar(placeholder_obj, skip);
                if (ucs != 0x20 && (ucs < 0x09 || ucs > 0x0D)) {
                    break;
                }
            }
            placeholder_len -= skip;
        }
        if (indent_len + placeholder_len > wrap_width) {
            PyErr_SetString(PyExc_ValueError, "placeholder too large for max width");
            return NULL;
        }
    }
    ...
    const char *initial_indent = option_utf8(initial_indent_obj, "initial_indent", "");
    if (initial_indent == NULL) {
        return NULL;
    }
    const char *subsequent_indent = option_utf8(subsequent_indent_obj, "subsequent_indent", "");
    if (subsequent_indent == NULL) {
        return NULL;
    }
    const char *placeholder = option_utf8(placeholder_obj, "placeholder", " [...]");
    if (placeholder == NULL) {
        return NULL;
    }

    wcwidth_wrap_opts_t opts = WCWIDTH_WRAP_OPTS_DEFAULT;
    opts.width = wrap_width;
    opts.control_codes = (wcwidth_control_mode_t)mode;
    opts.tabsize = tabsize;
    opts.ambiguous_width = ambiguous_width;
    opts.expand_tabs = expand_tabs != 0;
    opts.replace_whitespace = replace_whitespace != 0;
    opts.break_long_words = break_long_words != 0;
    opts.break_on_hyphens = break_on_hyphens != 0;
    opts.drop_whitespace = drop_whitespace != 0;
    opts.propagate_sgr = propagate_sgr != 0;
    opts.fix_sentence_endings = fix_sentence_endings != 0;
    opts.max_lines = max_lines;
    opts.initial_indent = initial_indent;
    opts.subsequent_indent = subsequent_indent;
    opts.placeholder = placeholder;
    opts.term_program = term_program;

    char *out = NULL;
    size_t out_len = 0;
    if (wrap_u8(text, (size_t)text_len, &opts, &out, &out_len) < 0) {
        PyErr_NoMemory();
        return NULL;
    }
    ...
    for (size_t i = 0; i < out_len; i++) {
        if (out[i] == '\n') {
            PyObject *line = PyUnicode_FromStringAndSize(line_start,
                                                         (Py_ssize_t)(out + i - line_start));
            if (line == NULL || PyList_Append(result, line) < 0) {
                Py_XDECREF(line);
                Py_DECREF(result);
                free(out);
                return NULL;
    ...
    if (line_start < out + out_len) {
        PyObject *line = PyUnicode_FromStringAndSize(line_start,
                                                     (Py_ssize_t)(out + out_len - line_start));
        if (line == NULL || PyList_Append(result, line) < 0) {
            Py_XDECREF(line);
            Py_DECREF(result);
            free(out);
...
static PyObject *
clip_impl(PyObject *self, PyObject *args, PyObject *kwargs)
{
    PyObject *text_obj;
    Py_ssize_t start;
    Py_ssize_t end;
    ...
    int propagate_sgr = 1;
    PyObject *control_codes_obj = NULL;
    PyObject *overtyping_obj = NULL;
    PyObject *term_program_obj = NULL;
    static char *kwlist[] = {"text", "start", "end", "fillchar", "tabsize",
                             "ambiguous_width", "propagate_sgr", "control_codes",
                             "overtyping", "term_program", NULL};

    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "Unn|OiipOOO", kwlist,
                                     &text_obj, &start, &end, &fillchar_obj, &tabsize,
                                     &ambiguous_width, &propagate_sgr, &control_codes_obj,
                                     &overtyping_obj, &term_program_obj)) {
        return NULL;
    }
    ...
    Py_ssize_t text_len;
    if (unicode_to_utf8(text_obj, &text, &text_len) < 0) {
        return call_pure_function("wcwidth._clip", "clip", args, kwargs);
    }
    const char *fillchar;
    size_t fillchar_len;
    ...
    int overtyping = -1;
    if (overtyping_obj != NULL && overtyping_obj != Py_None) {
        overtyping = PyObject_IsTrue(overtyping_obj);
        if (overtyping < 0) {
            return NULL;
        }
    }
    ...
    size_t out_len = 0;
    char *out = clip_u8(text, (size_t)text_len, (size_t)start, (size_t)end,
                        (wcwidth_control_mode_t)mode, tabsize, ambiguous_width,
                        term_program, propagate_sgr != 0, overtyping,
                        fillchar, fillchar_len, &out_len, &error);
    if (out == NULL) {
...
static PyMethodDef module_methods[] = {
    ...
    {"wrap", (PyCFunction)wrap_impl, METH_VARARGS | METH_KEYWORDS,
     "wrap(text, width=70, *, ...) -> list[str]\n\n"
     "Wrap a paragraph of text, returning a list of lines."},
    {"clip", (PyCFunction)clip_impl, METH_VARARGS | METH_KEYWORDS,
     "clip(text, start, end, *, ...) -> str\n\n"
     "Clip text to a visible column range, preserving terminal sequences."},
